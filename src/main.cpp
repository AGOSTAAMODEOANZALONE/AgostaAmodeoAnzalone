 ...
 *
 * Run:
 *   ./build/bin/astralog_processing --csv
 * input/telemetry/export_sat_alpha_small.csv
 */
...
int main(int argc, char *argv[]) {
  ...
  std::string rules_path =
      get_arg(argc, argv, "--rules", "input/rules_SAT_ALPHA.json");
  std::string sensors_path =
      get_arg(argc, argv, "--sensors", "input/sensors_SAT_ALPHA.yaml");
  ...
  std::string batch_strategy_str =
      get_arg(argc, argv, "--batch-strategy", "count");
  int batch_size = get_int_arg(argc, argv, "--batch-size", 1000);
  double batch_interval =
      get_double_arg(argc, argv, "--batch-interval", 5000.0);
  ...
  try {
    batch_config.strategy = astralog::string_to_strategy(batch_strategy_str);
    ...
    std::cerr << "[main] ERROR: invalid batch strategy: " << e.what() << "\n";
    return 1;
  }
  ...
  try {
    ...
    std::cerr << "[rules] Loaded " << rules.size() << " rule(s) from "
              << rules_path << "\n";
  } catch (const std::exception &e) {
  ...
  auto line_offsets =
      astralog::build_line_offsets(csv_file.data, csv_file.size);
  auto scan_end = std::chrono::high_resolution_clock::now();
  double scan_ms =
      std::chrono::duration<double, std::milli>(scan_end - scan_start).count();

  std::cerr << "[scan] Indexed " << line_offsets.size() << " data lines in "
            << scan_ms << " ms\n";

  // ── 5. Parallel CSV parsing ─────────────────────────────────────────
  auto parse_start = std::chrono::high_resolution_clock::now();
  uint64_t total_valid = 0, total_invalid = 0;
  auto records = astralog::parse_csv_parallel(csv_file, line_offsets, token_map,
  ...
  std::cerr << "[parse] Parsed " << total_valid << " valid, " << total_invalid
            << " invalid rows in " << parse_ms << " ms\n";

  // ── 6. Create output directory and open output files ────────────────
  try {
    fs::create_directories(output_dir);
  } catch (const std::exception &e) {
    std::cerr << "[main] ERROR: failed to create output dir: " << e.what()
              << "\n";
    return 1;
  }
  ...
  auto process_batch = [&](std::vector<astralog::ParsedRecord> &batch,
    ...
    auto result = astralog::process_pipeline(batch, rules, token_map,
                                             total_sensors, pipeline_state);

    auto batch_t1 = std::chrono::high_resolution_clock::now();
    double batch_ms =
        std::chrono::duration<double, std::milli>(batch_t1 - batch_t0).count();
    ...
    std::cerr << "[batch] Batch " << std::setfill('0') << std::setw(3)
              << batch_num << ": " << batch.size() << " records → "
              << result.num_nominal << " nominal, " << result.num_anomalies
              << " anomalous (" << batch_ms << " ms)\n";
  };
  ...
  double throughput_mrows =
      (total_valid + total_invalid) / (total_ms / 1000.0) / 1e6;
  ...
  std::cerr
      ...
      << "=============================================================\n"
      << "  Total lines processed: " << (total_valid + total_invalid) << "\n"
