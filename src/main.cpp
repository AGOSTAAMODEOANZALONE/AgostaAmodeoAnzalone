...
void print_usage(const char* prog) {
    std::cerr
        ...
        << "  --batch-interval <ms>     Batch interval for time strategy in ms "
           "(default: 5000)\n"
        << "  --benchmark               Disable batch audit files and per-batch "
           "phase logs\n"
        << "\n"
...
int main(int argc, char* argv[]) {
    ...
    bool        benchmark_mode = has_flag(argc, argv, "--benchmark");
    ...
    try {
        ...
        batch_config.write_audit_files = !benchmark_mode;
    ...
    std::cerr
        ...
        << "  Benchmark mode:   " << (benchmark_mode ? "ON" : "OFF") << "\n"
        << "  Batch strategy:   "
    ...
    // Build inverse token map: token (int) → sensor_id (string)
    auto inverse_token_map = astralog::build_inverse_token_map(token_map);
    auto token_view_map = astralog::build_token_view_map(inverse_token_map);
    ...
    auto prepared_rules = astralog::prepare_rules(rules, token_map, total_sensors);
    std::cerr << "[rules] Prepared: "
              << prepared_rules.threshold_count << " threshold, "
              << prepared_rules.step_diff_count << " step_diff, "
              << prepared_rules.stateful_count << " stateful, "
              << prepared_rules.correlation_rules.size() << " correlation\n";
    ...
    astralog::BatchAccumulator accumulator(batch_config, inverse_token_map);
    ...
    auto process_batch = [&](std::vector<astralog::ParsedRecord>& batch,
        ...
        auto result = astralog::process_pipeline(
            batch, prepared_rules, inverse_token_map,
            pipeline_state, !benchmark_mode);
    ...
    // Feed all valid records through the batch accumulator. Flush only at
    // timestamp boundaries so one timestamp is never split across batches.
    ...
    std::cerr
        ...
        << "  Batch audit files:     "
            << (batch_config.write_audit_files
                ? batch_config.batch_output_dir
                : std::string("disabled (--benchmark)")) << "\n"
        << "-------------------------------------------------------------\n"
