 ...
 *     as a side-effect of flush() for traceability and debugging, unless
 *     disabled for benchmark runs.
...
struct BatchConfig {
    ...
    bool          write_audit_files;  ///< Disable for clean benchmark timing
...
public:
    explicit BatchAccumulator(const BatchConfig& config,
        ...
        : config_(config)
        , inverse_token_map_(inverse_token_map)
        , batch_number_(0)
...
private:
    BatchConfig                config_;
    std::vector<std::string>   inverse_token_map_;
    ...
    void write_batch_file(const std::vector<ParsedRecord>& batch) {
        ...
        for (const auto& rec : batch) {
            ofs << rec.timestamp << ",";
            if (rec.sensor_token >= 0 &&
                static_cast<size_t>(rec.sensor_token) < inverse_token_map_.size()) {
                ofs << inverse_token_map_[rec.sensor_token];
            } else {
                ofs << "UNKNOWN";
            }
            ofs << "," << rec.value << "\n";
