 ...
 */
inline bool tokenize_csv_line(std::string_view line, std::string_view &f0,
                              std::string_view &f1, std::string_view &f2,
                              std::string_view &f3) noexcept {
  // Strip trailing \r or \n
  ...
  if (line.find(',', c3 + 1) != std::string_view::npos)
    return false;
...
inline bool validate_schema(std::string_view f0, std::string_view f1,
  ...
  if (f0.empty() || f1.empty() || f2.empty() || f3.empty())
    return false;
  auto is_corrupt = [](std::string_view s) {
    return s == "ERR" || s == "CORRUPT";
  };
  return !is_corrupt(f0) && !is_corrupt(f1) && !is_corrupt(f2) &&
         !is_corrupt(f3);
}
 ...
 */
inline std::vector<ParsedRecord>
parse_csv_parallel(const MappedFile &csv_file,
                   const std::vector<size_t> &line_offsets,
                   const std::unordered_map<std::string, int> &token_map,
                   uint64_t &total_valid, // maybe 64 is too much
...
#pragma omp for schedule(static)
    for (size_t i = 0; i < total_lines; ++i) {
      std::string_view line =
          get_line(csv_file.data, csv_file.size, line_offsets[i]);
      ...
      rec.sensor_id = std::string(f_sensor);
      rec.value = value;

      auto it = token_map.find(rec.sensor_id);
      rec.sensor_token = (it != token_map.end()) ? it->second : -1;
  ...
  for (int t = 0; t < num_threads; ++t)
    total_size += thread_records[t].size();
